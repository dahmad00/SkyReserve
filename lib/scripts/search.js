...
var dateDeparture = document.getElementById('departure')
var dateArrival = document.getElementById('arrival')
dateDeparture.min = new Date().toISOString().split("T")[0];
dateDeparture.min = new Date().toISOString().split("T")[0];
...
function searchQuery() {
    ...
    if (from == '') {
        ...
        alert("Destination and Arrival location can not be same ")
        return
    } 
    else {
        dep_date = new Date(departure)
        arr_date = new Date(arrival) 

        console.log(dep_date)
        console.log(arr_date)

        if (dep_date > arr_date && roundTrip) {
